...
Parâmetros
---
- `entity` (string): o nome da entidade que a página gerencia. Será usada para compor textos da UI.
- `output_path` (string): o caminho relativo onde o arquivo Dart será criado (por exemplo: `lib/features/providers/presentation`). O gerador NÃO deve criar o arquivo; esse parâmetro serve para que o aluno saiba onde salvar o código retornado.
- `output_path` (string): o caminho relativo onde o arquivo Dart será criado (por exemplo: `lib/features/providers/presentation`). O gerador deve criar o arquivo neste caminho; o nome do arquivo deve ser `entity_list_page.dart` salvo dentro do `output_path` informado.
- `entity_import_path` (string): o caminho de import da entidade (por exemplo: `package:meu_app/models/provider.dart`). Use este valor apenas para compor imports no código se necessário.
